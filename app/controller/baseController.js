...
class BaseController extends Controller {
  ...
  async findOne(options, modelName = null) {
    options.raw = true;
    return await this.ctx.model[modelName || this.Name].findOne(options)
      .then(obj => {
        return obj;
      })
      .catch(() => {
  ...
  async destroy(options, modelName = null) {
    return await this.ctx.model[modelName || this.Name].destroy(options)
      .then(() => {
        return true;
      })
      .catch(err => {
  ...
  async update(values, options, modelName = null) {
    return await this.ctx.model[modelName || this.Name].update(values, options)
      .then(() => {
        return true;
      })
      .catch(err => {
  ...
  async _compute(fields, method, where, modelName = null) {
    return await this.ctx.model[modelName || this.Name][method](fields, where)
      .then(() => {
        return true;
      })
      .catch(err => {
  ...
  async count(where, modelName = null) {
    return await this.ctx.model[modelName || this.Name].count({ where })
      .then(num => {
        return num;
      })
      .catch(() => {
