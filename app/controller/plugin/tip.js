...
class TipController extends Controller {
  ...
  async reward() {
    ...
    if (_user) {
      ...
      const isOk = await this.ctx.model.transaction(async t => {
        ...
        if (!await this.create({ userId: toId, fromId, shell: num, type: 3 }, t, 'Trade')) {
  ...
  async tradeLogs() {
    ...
    for (const item of items) {
      item.moment = moment(item.addDate)
        .format('MM/DD HH:mm');
      if (item.fromId) {
  ...
  async _isSubscribe() {
    ...
    if (user) {
      ...
      if (token) {
        ...
        const obj = await this.ctx.http.get(url)
          .then(obj => {
            if (obj && obj.errcode) {
