...
class PostController extends Controller {
  ...
  async Release() {
    ...
    const _userId = await this.getUserId();
    const post = await this.ctx.model.transaction(async t => {
      ...
      if (!_hasFollow) {
        ...
        if (!await this.increment('score', { userId: _userId, topicId }, 5, t, 'TopicFollow')) {
          throw Error('score');
        }
      }
      return _post;
      ...
      request(url, function(error, response, body) {
        if (!error && (response && response.statusCode === 200)) {
          ...
          if (/(https):\/\/(mp.weixin.qq.com)[-A-Za-z0-9+&@#/%=~_|]*/.test(url)) {
            title = $('meta[property="twitter:title"]')
              .attr('content');
            imageSrc = $('meta[property="twitter:image"]')
              .attr('content');
          } else if (/(https):\/\/(juejin.im)[-A-Za-z0-9+&@#/%=~_|]*/.test(url)) {
            title = $('title')
              .text();
            imageSrc = $('link[rel="apple-touch-icon"]')
              .attr('href');
          } else if (/(https):\/\/([www.jianshu.com|zhuanlan.zhihu.com])[-A-Za-z0-9+&@#/%=~_|]*/.test(url)) {
            title = $('meta[property="og:title"]')
              .attr('content');
            imageSrc = $('meta[property="og:image"]')
              .attr('content');
          }
          if (title) {
            resolve({
            ...
            if (/(https):\/\/(mp.weixin.qq.com)[-A-Za-z0-9+&@#/%=~_|]*/.test(url)) {
              ...
              for (const img of imgs) {
                const src = $(img)
                  .attr('data-src');
                $(img)
                  .attr('src', `${web}/api/public/image/${encodeURIComponent(src)}`);
              }
              ...
              for (const link of links) {
                ...
                if (src) {
                  $(link)
                    .attr('href', `${web}/api/public/image/${encodeURIComponent(src)}`);
                }
              }
              const imgs = $('img')
                .toArray();
              ...
              for (const img of imgs) {
                ...
                $(img)
                  .attr('src', `${web}/api/public/image/${encodeURIComponent('https:' + src)}`);
              }
              $('script[src]')
                .remove();
  ...
  async listForSchool() {
    ...
    const where = this.literal(`exists(select id from users where school = '${school}' and id = \`post-recommend\`.userId) ${begin ? ` and senDate ${isRefresh ? '>' : '<'} ${begin}` : ''}`);
    const items = [];
    const _items = await this.findAll([ 'postId', 'userId' ], where, [ [ 'senDate', 'DESC' ] ], pageIndex, pageSize, 'PostRecommend');
    for (const _item of _items) {
      const user = await this.getUserInfo(_item.userId);
  ...
  async getComments() {
    ...
    if (!items) {
      ...
      for (const _item of items) {
        ...
        _item.moment = moment(_item.senDate)
          .fromNow();
        if (likeType !== 0) {
